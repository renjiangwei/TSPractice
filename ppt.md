...
- hooks 可复用逻辑
- JSX封装可复用组件逻辑。不用为了封装而封装，有时候写的写的觉得这一片好像都一样或者相似，就可以放一起；但是如果是大篇幅的内容就不要去封装了，假如项目迭代中其中一个情况要做修改，此时兼容多种复用也是麻烦事。
...
开始可能会觉得写TS是一件浪费时间的事情，那说明运用不够熟练.提前定义好类型，编写代码有提示，很多bug可以直接报警告，而不是上下文翻了半天才发现，某个对象访问中间少了一层  
```typescript
const state = {
  e: {
    f: true
